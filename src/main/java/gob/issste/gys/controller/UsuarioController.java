...
//        String originalData = encryptionService.decrypt(encryptedUser, EncryptionService.BINDING_KEY);
        ...
        try {
            ...
            if (_usuario != null) {
                ...
                // Delete Permissions
                usuarioRepository.removePermissions(usuario.getIdUsuario());
                perfilRepository.removePerfilesToUser(usuario.getIdUsuario());
                usuarioRepository.removeCentTrabForUsu(usuario.getIdUsuario());
                // Add Permissions
                for (Perfil p : usuario.getPerfiles()) {
                    perfilRepository.addPerfilToUser(usuario.getIdUsuario(), p.getIdPerfil());
                    for (Opcion o : p.getOpciones()) {
                        usuarioRepository.savePermissions(usuario.getIdUsuario(), p.getIdPerfil(), o.getIdOpcion(), o.getIdNivelAcceso());
                    }
                }
                for (DatosAdscripcion ct : usuario.getCentrosTrabajo()) {
                    usuarioRepository.saveCentTrabForUsu(usuario.getIdUsuario(), ct.getClave(), usuario.getId_usuario());
                }
                platformTransactionManager.commit(status);
                return ResponseHandler.generateResponse("El usuario ha sido modificado exitosamente", HttpStatus.OK, null);

            } else {
                return ResponseHandler.generateResponse("No se pudo encontrar el usuario = " + usuario.getClave(), HttpStatus.NOT_FOUND, null);
            }
        } catch (Exception e) {
            platformTransactionManager.rollback(status);
            return ResponseHandler.generateResponse("Error al actualizar el usuario", HttpStatus.INTERNAL_SERVER_ERROR, null);
        }
    }
...
//            String encryptedUsers = encryptionService.encrypt(jsonString, EncryptionService.BINDING_KEY);
    ...
    @Operation(summary = "Obtiene el usuario mediante el ID del Sistema",
            description = "Obtiene el usuario mediante el ID del Sistema",
            tags = {"Usuario"})
    @GetMapping("/usuarios/{data}")
    public ResponseEntity<Object> getUsuarioById(
            @Parameter(description = "El ID del usuario consultado.", required = true)
            @PathVariable("data") String data,
            @Parameter(description = "Boolean para indicar si se incluyen los Perfiles", required = true)
            @RequestParam(value = "conPerfiles", required = true) Boolean conPerfiles
    ) {
...
//                String encryptedUser = encryptionService.encrypt(jsonString, EncryptionService.BINDING_KEY);
                ...
                return ResponseHandler.generateResponse("No se encontró al usuario seleccionado", HttpStatus.NOT_FOUND, null);

            }
        } catch (Exception e) {
            return ResponseHandler.generateResponse("Error inesperado al obtener el usuario", HttpStatus.INTERNAL_SERVER_ERROR, e.getMessage());
        }

    }

    @Operation(summary = "Elimina el Usuarios con el ID indicado del Sistema",
            description = "Elimina el Usuarios con el ID indicado del Sistema",
            tags = {"Usuario"})
...
//			Usuario usuario = usuarioRepository.findById(id);
            ...
            return ResponseHandler.generateResponse("Usuario eliminado con éxito", HttpStatus.OK, null);
...
//			@ApiResponse(responseCode = "400", description = "Invalid username/password supplied", content = @Content) })
    ...
    @Operation(summary = "Actualiza el password del usuario",
            description = "Actualiza la contraseña de usuario seleccionado",
            tags = {"Usuario"})
    @PutMapping("/usuarios/pwd/{data}")
    public ResponseEntity<Object> updatePasswordUsuario(
            @Parameter(description = "Objeto encriptado con el id y la nueva contraseña",
                    required = true)
...
//            String decryptedJson = encryptionService.decrypt(data, EncryptionService.BINDING_KEY);
            ...
            if (result == 0) {
                return ResponseHandler.generateResponse("El usuario no fué encontrado", HttpStatus.NOT_FOUND, null);
            } else {
                return ResponseHandler.generateResponse("Se actualizo la contraseña el usuario en el Sistema", HttpStatus.OK, null);
            }
        } catch (Exception e) {
            return ResponseHandler.generateResponse("No se actualizo la contraseña del usuario seleccionado", HttpStatus.INTERNAL_SERVER_ERROR, null);
        }
    ...
    @PutMapping("/usuarios/active/{data}")
    public ResponseEntity<Object> updateActiveUser(
            @Parameter(description = "Objeto encriptado con el id y el estado del usuario", required = true)
            @PathVariable(required = true) String data
    ) throws SQLException {
        try {
//            String decryptedJson = encryptionService.decrypt(data, EncryptionService.BINDING_KEY);
            ...
            if (active) {
                usuarioRepository.updateActive(true, idUsuario);
                usuarioRepository.updateAttemps(0, idUsuario);
